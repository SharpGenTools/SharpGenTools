...
using SharpGen.Platform.Clang.Extensions;
...
namespace SharpGen.Platform.Clang
{
    public sealed partial class PInvokeGenerator : IDisposable
    {
        ...
        private readonly IClangSharpHost _host;
        ...
        public PInvokeGenerator(PInvokeGeneratorConfiguration config, IClangSharpHost host)
        {
            if (config is null)
            {
        ...
        public string GetCursorName(NamedDecl namedDecl)
        {
        ...
        public string GetCursorQualifiedName(NamedDecl namedDecl, bool truncateFunctionParameters = false)
        {
        ...
        private string GetTypeName(Cursor cursor, Cursor context, Type type, out string nativeTypeName)
        {
            ...
            else if (type is ClangSharp.BuiltinType)
            {
                switch (type.Kind)
                {
        ...
        private void GetTypeSize(Cursor cursor, Type type, ref long alignment32, ref long alignment64, out long size32, out long size64)
        {
            ...
            else if (type is RecordType recordType)
            {
                ...
                foreach (var declaration in recordType.Decl.Decls)
                {
                    ...
                    if ((fieldAlignment32 == -1) || (alignment32 < 4))
                    {
                        fieldAlignment32 = fieldSize32 > 0 ? Math.Min(alignment32, fieldSize32) : alignment32;
                    }

                    if ((fieldAlignment64 == -1) || (alignment64 < 4))
                    {
                        fieldAlignment64 = fieldSize64 > 0 ? Math.Min(alignment64, fieldSize64) : alignment64;
                    }
        ...
        private bool IsExcluded(Cursor cursor, out bool isExcludedByConflictingDefinition)
        {
            ...
            bool IsExcludedByFile(Cursor cursor)
            {
                ...
                if ((expansionFile == file) && (expansionLine == line) && (expansionColumn == column))
                {
            ...
            bool IsIncludedFileOrLocation(Cursor cursor, CXFile file, CXSourceLocation location)
            {
                ...
                return _host.IsIncludedFileOrLocation(cursor, file, location);
            }
